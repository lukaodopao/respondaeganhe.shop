...
const requiredParams = ['utm_source', 'utm_campaign', 'utm_medium', 'utm_content', 'xcod', 'sck'];
...
function getCompleteParams() {
  ...
  const originalUtmCampaign = getParam('utm_campaign');
  const utmSource = getParam('utm_source');
  const completeParams = new URLSearchParams();
  completeParams.set('utm_source', utmSource);
  completeParams.set('utm_campaign', originalUtmCampaign.includes(MEDIUM_SEPARATOR) ? originalUtmCampaign : `${originalUtmCampaign}${MEDIUM_SEPARATOR}${utmMedium}`);
  completeParams.set('utm_medium', utmMedium);
  completeParams.set('utm_content', utmContent);
  completeParams.set('utm_term', utmTerm);
  const utmCampaign = completeParams.get('utm_campaign') ?? '';
  ...
  // BUYGOODS
  completeParams.set('subid', utmSource);
  completeParams.set('subid2', utmCampaign);
  completeParams.set('subid3', utmMedium);
  completeParams.set('subid4', utmContent);
  ...
  if (src != null && src !== '') {
    completeParams.set('src', src);
  }
  return completeParams;
}
function work() {
  saveParams();
  function rebuildLinks(needAValidation) {
    ...
    document.querySelectorAll('a').forEach(e => {
      ...
      if (needAValidation) {
        const alreadyHaveAllParams = requiredParams.every(param => e.href.includes(param));
        if (alreadyHaveAllParams) return;
      }
      const char = e.href.indexOf('?') === -1 ? '?' : '&';
      e.href += char + (completeParams?.toString() ?? '');
    });
  }
  function rebuildForms(needAValidation) {
    const completeParamsStr = encodeURIComponent(getCompleteParams()?.toString() ?? '');
    const forms = document.querySelectorAll('form');
    console.log('forms', forms);
    forms.forEach(form => {
      if (needAValidation) {
        const alreadyHaveAllParams = requiredParams.every(param => form.action.includes(param));
        if (alreadyHaveAllParams) return;
      }
  ...
  function onBodyUpdate() {
    ...
    const mutationCallback = (_mutationList, _observer) => {
      rebuildLinks(true);
      rebuildForms(true);
    };
